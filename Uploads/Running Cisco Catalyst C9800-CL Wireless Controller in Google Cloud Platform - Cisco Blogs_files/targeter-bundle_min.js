...
cdc.sso = (function (self) {
    let lib = "sso",
        ...
        cdcValidConfig = {
            prod: {
                validateLogin: "//sso.cisco.com/autho/apps/sso/validatessosession/cdc/cdcvalid.js?t=" + Math.random()
            },
            stage: {
                validateLogin: "//sso-nprd.cisco.com/autho/apps/sso/validatessosession/cdc/cdcvalid.js?t=" + Math.random()
            },
            dev: {
                validateLogin: "//sso-nprd.cisco.com/autho/apps/sso/validatessosession/cdc/cdcvalid.js?t=" + Math.random()
            }
    ...
    // check for ssoCookie existence, if it exists login is valid
    let isAuthenticated = () => checkCookie(ssoConfig[env].ssoCookieName);
    ...
    let fallbackCheck = () => {
        if (checkCookie('ObSSOCookie')) {
            let script = document.createElement('script');
            script.src = cdcValidConfig[env].validateLogin
    ...
    let init = () => {
        if (!isAuthenticated() && checkCookie("authorization")) {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) { 
                    setSSOCookie();
                    triggerListeners();
                } else if (xhr.readyState === 4 && xhr.status !== 200 && xhr.status !== 403) {
                    fallbackCheck();
                }
            };
    ...
    // temporary interface
    self.oamStatus = oamStatus;

    // API Aliases
    // Decommision in Janurary 2021
    let dummy = function () { return false; }
    self.idleTimer = 0;
    self.resynchInterval = 0;
    self.authStatusIsSynced = false;
    self.resynchInterval = dummy;
    self.initialSynch = dummy;
    self.keepSynch = dummy;
    self.nonSynchable = dummy;
    self.staySynched = dummy;
...
var Targeter = (function (window, undefined) {
  ...
  var ADOBE_ACCOUNTS = {
    ...
    TEST: {
      ...
      serverDomain: 'testciscosystemsinc.tt.omtrdc.net',
...
//
  ...
  var isReady = function(source) {
    ...
    var ready = typeof(visitor) !== 'undefined' // this ensures that the visitor namespace exists
